(() => {
  ...
  const ZTYPES = {
    walker: { hp: 42, spd: 0.72, dmg: 10, r: 0.28, atk: 0.85, scale: 0.92, see: 8, body: "#3d5a38", eye: "#9dff6a" },
    runner: { hp: 24, spd: 1.35, dmg: 8, r: 0.24, atk: 0.5, scale: 0.82, see: 10, body: "#6a4036", eye: "#ff6a4a" },
    brute: { hp: 220, spd: 0.55, dmg: 26, r: 0.4, atk: 1.15, scale: 1.25, see: 7, body: "#2a2e28", eye: "#ffcc44" },
    spitter: { hp: 32, spd: 0.82, dmg: 14, r: 0.26, atk: 1.5, scale: 0.88, see: 11, body: "#4a5c28", eye: "#d6ff44" },
  };
  const WALL = [
    null,
  ...
  function hurtZombie(z, dmg, dir, knock) {
    ...
    z.hit = 0.12;
    z.aggro = true;
    z.x += Math.cos(dir) * knock * 0.02;
    z.y += Math.sin(dir) * knock * 0.02;
    sfxHit();
  ...
  function spawnZombie(kind, x, y) {
    const t = ZTYPES[kind];
    G.zombies.push({
      kind, x, y, r: t.r, hp: t.hp, max: t.hp, spd: t.spd * rand(0.92, 1.05),
      dmg: t.dmg, atk: t.atk, scale: t.scale, see: t.see, body: t.body, eye: t.eye,
      cd: 0, hit: 0, dead: false, aggro: false, wander: rand(0, TAU), walk: rand(0, TAU),
    });
  }
  ...
  function updateZombies(dt) {
    ...
    for (const z of G.zombies) {
      ...
      const canSee = d <= z.see && los(z.x, z.y, p.x, p.y);
      if (canSee) z.aggro = true;
      else if (d > z.see * 1.35) z.aggro = false;

      let fx = 0, fy = 0, spd = z.spd;
      if (z.aggro) {
        fx = dx / d;
        fy = dy / d;
        const nx = z.x + fx * 0.45, ny = z.y + fy * 0.45;
        if (solid(nx, ny)) {
          if (!solid(z.x + fy * 0.45, z.y - fx * 0.45)) { fx = fy; fy = -dx / d; }
          else if (!solid(z.x - fy * 0.45, z.y + fx * 0.45)) { fx = -fy; fy = dx / d; }
        }
      } else {
        spd *= 0.28;
        z.wander += dt * 0.4;
        ...
        if (solid(z.x + fx * 0.4, z.y + fy * 0.4)) z.wander += 1.2;
      }
      for (const o of G.zombies) {
        if (o === z || o.dead) continue;
        const dd = hypot(z.x, z.y, o.x, o.y);
        if (dd < 0.5 && dd > 0) { fx -= (o.x - z.x) / dd * 0.25; fy -= (o.y - z.y) / dd * 0.25; }
      }
      const fl = Math.hypot(fx, fy) || 1;
      tryMove(z, (fx / fl) * spd * dt, (fy / fl) * spd * dt, z.r);
      if (!z.aggro) continue;
      if (z.kind === "spitter" && d < 9 && d > 1.6 && z.cd <= 0 && canSee) {
        z.cd = z.atk;
        const inv = 1 / d;
        G.spit.push({ x: z.x, y: z.y, vx: dx * inv * 6, vy: dy * inv * 6, life: 1.3, dmg: z.dmg });
